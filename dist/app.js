...
class GoogleFormApp {
    ...
    loadForms() {
        this.forms = LocalStorageHelper.getData(this.formKey) || [];
    }
    saveForms() {
    ...
    addField() {
        ...
        const type = prompt('Enter field type (text, radio, checkbox):');
    ...
    previewForm(formId) {
        const formContainer = document.getElementById('form-preview');
        const formPreviewContainer = document.getElementById('form-preview-container');
        if (!formContainer || !formPreviewContainer)
            return;
